...
function isRejectedCompanyName(value: string) {
  ...
  if (REJECTED_COMPANY_MARKERS.some((marker) => compact === marker)) {
    return true;
  }
...
function isAcceptedActivityDomain(value: string) {
  return ACTIVITY_DOMAINS.has(value);
}

function getLeadFingerprint(phone: string) {
